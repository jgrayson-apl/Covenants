  ...
  return declare([Evented], {
    ...
    initializeZoomToBookmark: function (view) {
      if(view.map.bookmarks && view.map.bookmarks.length > 0) {
        ...
        view.map.bookmarks.forEach(bookmark => {
          const bookmark_btn = domConstruct.create("button", { className: "btn btn-clear", innerHTML: bookmark.name });
    ...
    initializeTimeFilter: function (view, timeExtent) {
      ...
      const format_date = (date_time) => {
        return locale.format(date_time, { selector: "date", datePattern: "yyyy" });
      };
      ...
      const update_time_filter = () => {
        dom.byId("year-node").innerHTML = format_date(new Date(time_input.valueAsNumber));
